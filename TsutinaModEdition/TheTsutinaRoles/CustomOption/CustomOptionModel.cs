...
namespace TheTsutinaRoles.CustomOption
{
    public class CustomOption
    {
        ...
        public CustomOption(int id, string name, System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, bool isHidden, string format)
        {
            ...
            if (id > 0)
            {
                entry = TheTsutinaRolesPlugin.Instance.Config.Bind($"Preset{preset}", id.ToString(), defaultSelection);
                selection = Mathf.Clamp(entry.Value, 0, selections.Length - 1);
                if (options.Any(x => x.id == id))
                {
                    TheTsutinaRolesPlugin.Logger.LogInfo($"CustomOptionのId({id})が重複しています。");
                }
                if (Max < id)
                {
                    Max = id;
        ...
        public static void switchPreset(int newPreset)
        {
            ...
            foreach (CustomOption option in CustomOption.options)
            {
                ...
                option.entry =TheTsutinaRolesPlugin.Instance.Config.Bind($"Preset{preset}", option.id.ToString(), option.defaultSelection);
    ...
    class GameOptionsDataPatch
    {
        ...
        public static void Postfix(ref string __result)
        {
            ...
            int counter =TheTsutinaRolesPlugin.optionsPage =TheTsutinaRolesPlugin.optionsPage % numPages;
    ...
    public static class GameOptionsNextPagePatch
    {
        public static void Postfix(KeyboardJoystick __instance)
        {
            ...
            {
                TheTsutinaRolesPlugin.optionsPage = TheTsutinaRolesPlugin.optionsPage + 1;
            }
        }
    }
